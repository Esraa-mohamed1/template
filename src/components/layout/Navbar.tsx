    ...
    <nav className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-gray-100">
      <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-10 text-[11px] font-bold uppercase tracking-widest text-gray-400">
          ...
          <button
            onClick={() => onCategoryClick("Winter")}
            className="hover:text-brand-blue transition-colors cursor-pointer"
          >
            Winter Sale
          </button>
          {onBuilderClick && (
            <button
              ...
              className="text-blue-500 hover:text-blue-600 font-extrabold transition-colors cursor-pointer"
            >
              Builder ✨
            </button>
          )}
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="hidden md:flex items-center gap-2">
            <button
              onClick={onUserClick}
              className="p-2.5 hover:bg-gray-100 rounded-2xl transition-all text-gray-600"
            >
              <User size={20} />
            </button>
            <motion.button
              id="cart-icon-target"
              onClick={onCartClick}
              animate={{ scale: bump ? [1, 1.25, 1] : 1 }}
              transition={{ duration: 0.3, ease: [0.3, 0, 0.2, 1.6] }}
