...
const CartPage = ({
  ...
  // لو تعديل الكارت خلى الـ subtotal يقل عن الحد الأدنى بتاع الكوبون المطبق، شيله تلقائي
  useEffect(() => {
    if (appliedCoupon && subtotal < Number(appliedCoupon.min_order_amount)) {
      setAppliedCoupon(null);
      setCouponError(
        "السلة لم تعد تستوفي الحد الأدنى للطلب الخاص بهذا الكوبون",
      );
    }
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      if (!res.status) {
        setCouponError(res.message || "حدث خطأ أثناء التحقق من الكوبون");
        setAppliedCoupon(null);
        return;
      }
      ...
      const msg =
        err instanceof HttpError
          ? (err.body as any)?.message || "تعذر الاتصال بالخادم، حاول مرة أخرى"
            ...
            <div className="sticky top-32">
              <div className="bg-brand-dark text-white rounded-[2.5rem] p-8 shadow-2xl overflow-hidden relative">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-[11px] text-gray-400">
                            {appliedCoupon.type === "percentage"
                              ? `خصم ${Number(appliedCoupon.value)}%`
                              : `خصم $${Number(appliedCoupon.value).toFixed(2)}`}
                          </p>
                        </div>
                      </div>
                      <button
                ...
                <p className="text-[10px] text-gray-500 text-center mt-6">
                  Secure Checkout • Tax calculated at next step
                </p>
              </div>
            </div>
          </div>
