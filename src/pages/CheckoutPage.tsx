...
const CheckoutPage = ({
  ...
  // ── Shipping method + Governorates / Branches ──
  ...
  // فروع المحافظة المختارة
  const branches = useMemo(
    () => selectedGovernorate?.branches ?? [],
    [selectedGovernorate],
  );

  // هل اختيار الفرع إجباري؟ بس لما يكون فيه أكتر من فرع واحد
  const isBranchRequired = branches.length > 1;

  // ── حساب سعر الشحن حسب نوع الطريقة ──
  ...
  // ── جلب طريقة الشحن + المحافظات وفروعها ──
  useEffect(() => {
  ...
  // ── تجهيز كل الداتا اللي هتتبعت للباك اند ──
  ...
  // ── تنفيذ الأوردر: بينده orderService.create وبعدين onComplete ──
