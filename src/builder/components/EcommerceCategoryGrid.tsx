...
export default function EcommerceCategoryGrid({
  ...
  // Map category height classes
  const sizeMap: Record<string, string> = {
    small: 'h-20',
    medium: 'h-28',
    large: 'h-36',
  };
  const imageHeightClass = sizeMap[imageSize] || 'h-28';
      ...
      <section style={{ backgroundColor: bgColor }} className="w-full py-10 px-6 transition-colors duration-300">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
            {CATEGORIES.map(cat => (
              <button key={cat.id} style={{ backgroundColor: cardBg }} className="flex flex-col items-center gap-2 p-3 rounded-2xl border border-slate-100 hover:shadow-md hover:-translate-y-0.5 transition-all group cursor-pointer">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl" style={{ backgroundColor: cat.color }}>
                  {cat.icon}
                </div>
                <span className="text-[11px] font-black group-hover:opacity-85 transition-colors text-center" style={{ color: textColor }}>{cat.label}</span>
              </button>
            ))}
          </div>
        </div>
  ...
  if (layoutFrame === '2-col-large') {
    return (
      <section style={{ backgroundColor: bgColor }} className="w-full py-10 px-6 transition-colors duration-300">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-2xl font-black mb-6" style={{ color: textColor }}>{sectionTitle}</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {CATEGORIES.slice(0, 4).map(cat => (
              <button key={cat.id} style={{ backgroundColor: cardBg }} className="relative flex items-center gap-4 p-5 rounded-2xl border border-slate-100 hover:shadow-lg hover:-translate-y-1 transition-all overflow-hidden group cursor-pointer text-left">
                ...
                <div className="relative z-10">
                  <p className="text-base font-black" style={{ color: textColor }}>{cat.label}</p>
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5" style={{ color: `${textColor}99` }}>Browse products across all categories</p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {CATEGORIES.map(cat => (
            <button key={cat.id} style={{ backgroundColor: cardBg }} className="relative flex flex-col overflow-hidden rounded-2xl border border-slate-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group cursor-pointer text-left">
              ...
              <div className="p-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm font-black" style={{ color: textColor }}>{cat.label}</p>
